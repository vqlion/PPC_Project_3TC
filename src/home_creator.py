...
def handler_alrm(sig, frame):
    #upon receiving termination signal, the process in turn sends a termination signal to every home
    global process_list
...
def create_homes(number_of_homes, homes_type):
    ...
    mq = sysv_ipc.MessageQueue(MESSAGE_QUEUE_KEY, sysv_ipc.IPC_CREAT) #create the message queue for the homes
